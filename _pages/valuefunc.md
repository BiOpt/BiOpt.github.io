...
where $x\in\mathbb{R}^{n_x},y\in\mathbb{R}^{n_y}$ and $g:\mathbb{R}^{n_x\times n_y}\rightarrow \mathbb{R}^{n_g}$. Detailed descriptions of usage of this toolbox can be found in  the [menu-of-BiOpt](\files\menu-of-BiOpt.pdf) and  $\texttt{OptValFunc}$ can be [download here](\files\OptValFunc.zip), in which there are two tools:
